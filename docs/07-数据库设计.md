...
用户(User) ──N:N── 用户角色(UserRole) ──N:N── 角色(Role)
                            ...
                            ┌─────────────────────┼─────────────────────┐
                            ▼                     ▼                     ▼
                     角色权限(RolePermission)  角色菜单(RoleMenu)   角色数据权限部门(RoleDataScopeDept)
                            │                     │                     │
                            ▼                     ▼                     ▼
                       权限(Permission)       菜单(Menu)            部门(Department)

用户(User) ──N:1── 部门(Department)
菜单(Menu) ──N:1── 权限(Permission)（MenuId 关联）
...
| Type | NVARCHAR | 默认 Menu | 类型：Directory/Menu/Button |
| Sort | INT | 默认 0 | 排序 |
| IsEnabled | BIT | 默认 1 | 是否启用 |
...
| ---- | ---- | ---- | ---- |
| Id | INT | PK | 主键 |
| Code | NVARCHAR | NOT NULL | 权限编码（`module:resource:action`） |
| Name | NVARCHAR | NOT NULL | 权限名称 |
| Type | NVARCHAR | 默认 Button | 类型：Menu/Button/Api |
| Description | NVARCHAR | NULL | 描述 |
| MenuId | INT | NULL, FK | 关联菜单 ID |
| CreatedAt / UpdatedAt | DATETIME2 | 自动 | 时间戳 |

### 4.6 Dictionaries（字典表）
...
| ---- | ---- | ---- |
| RoleId | INT | PK, FK → Roles.Id |
| PermissionId | INT | PK, FK → Permissions.Id |

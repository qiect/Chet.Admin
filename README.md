...
| 模块 | 说明 |
| ---- | ---- |
| 认证登录 | 注册、登录、刷新令牌、验证码、登录锁定、密码过期策略 |
| 用户管理 | CRUD、密码强度、分配角色、数据权限过滤 |
| 角色管理 | CRUD、权限分配、菜单分配、5 种数据权限范围 |
| 菜单管理 | 树形 CRUD、动态路由生成、目录/菜单/按钮 |
| 部门管理 | 树形 CRUD、组织架构 |
| 权限管理 | CRUD、按钮级权限码 |
| 字典管理 | CRUD、`useDict` 联动业务表单 |
| 仪表盘 | 统计卡片、SVG 趋势图、最近操作 |
| 操作日志 | 中间件自动记录、分页查询、清理 |
| 通知公告 | 全局公告、个人通知、未读计数 |
| 文件上传 | 本地存储、上传/下载/删除 |
| 在线用户 | 登录追踪、强制下线 |
| 个人中心 | 资料修改、密码修改、头像上传 |
